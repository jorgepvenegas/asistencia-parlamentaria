...
export const politiciansTable = sqliteTable(
  'politicians',
  {
...
export const partiesTable = sqliteTable(
  'parties',
  {
    id: int().primaryKey({ autoIncrement: true }),
    name: text().notNull(),
...
export const attendanceMonthlyTable = sqliteTable(
  'attendance_monthly',
  {
    id: int().primaryKey({ autoIncrement: true }),
    politicianId: int()
  ...
  (table) => [uniqueIndex('politician_month_idx').on(table.politicianId, table.year, table.month)]
);

export const attendanceYearlyTable = sqliteTable(
  'attendance_yearly',
  {
    id: int().primaryKey({ autoIncrement: true }),
    politicianId: int()
  ...
  (table) => [uniqueIndex('politician_year_idx').on(table.politicianId, table.year)]
);
